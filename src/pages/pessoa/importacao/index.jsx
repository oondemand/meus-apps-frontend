...
export const ImportPessoasPage = () => {
  const [open, setOpen] = useState([0]);

  const { mutateAsync: importPessoasMutation, isPending } = useMutation({
    mutationFn: async ({ files }) =>
      await PessoaService.importarPessoas({ files }),
    onSuccess() {
      queryClient.refetchQueries(["listar-pessoas"]);
      queryClient.invalidateQueries(["list-pessoas-importados"]);
      toaster.create({
  ...
  const { data } = useQuery({
    queryKey: ["list-pessoas-importados"],
    queryFn: async () => {
      const { data } = await api.get("/importacoes?tipo=pessoa&pageSize=5");
      return data;
    },
        ...
        <Box>
          <FileUploadRoot
            ...
            onFileAccept={async (e) => {
              await importPessoasMutation({ files: e.files });
            }}
            maxFiles={1}
          >
            <FileUploadTrigger>
                      ...
                      {importacao?.detalhes && (
                        <Box>
                          <Flex mt="1" gap="8">
                            ...
                            <Box>
                              <Text fontSize="sm" color="gray.600">
                                Total de novas pessoas
                              </Text>
                              <Text fontSize="2xl" fontWeight="bold">
                                {data
                                  ? importacao.detalhes?.novasPessoas
                                  : "..."}
                              </Text>
                            </Box>
